...
void main() {
  ...
  test('never imports the vendor shell', () {
    final offenders = [
      for (final file in _sources())
        if (file.readAsStringSync().contains('package:aangan_feature_vendor'))
          file.path,
    ];
  ...
  test('the dependency is absent from the manifest too', () {
    ...
    expect(
      File('pubspec.yaml').readAsStringSync().contains('aangan_feature_vendor'),
      isFalse,
    );
  });
  ...
  /// A stage is done when somebody at Aangan checked it.
  ///
